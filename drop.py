...
class Drop(commands.Cog):
    ...
    async def bid(self, ctx, item_id:int, dkp:int):
        ...
        if(dkp < 100):
            await ctx.send(player.id+', the minimum bid is 100.')
        ...
        elif ((dkp > (highest_bids[item_id-1] + 24)) or ((dkp > highest_bids[item_id-1]) and (highest_bids[item_id-1] > (player.dkp - 25)))):
            if (countdown < 15):
                countdown = 15
            ...
            await ctx.send(player.id+', the minimum increment is 25.')
